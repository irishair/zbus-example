...
use log::LevelFilter;
use zbus::{zvariant::ObjectPath, zvariant::OwnedObjectPath, dbus_proxy, Connection, Result};
...
async fn main() -> anyhow::Result<()> {
  ...
  for (name, path, stat) in hnodes.iter() {
    log::debug!("HirteManager: ListNodes:[{}|{}|{}]", name.as_str(), path.as_str(), stat.as_str());
  }
  ...
  let j1 = hirte_node.start_unit("v2x-server.service", "replace").await;
  if j1.is_err() {
  ...
  let j2 = hirte_node.stop_unit("v2x-server.service", "replace").await;
  if j2.is_err() {
